...
def test_game_be_004_1_s2_invalid_card_index_raises_rule_violation():
    # GIVEN a GameState where the active player's hand has 2 cards (indices 0, 1)
    active = Player(id="p1", hp=30, mana=10, mana_slots=10, hand=[2, 3], deck=[])
    opponent = Player(id="p2", hp=30, mana=0, mana_slots=0, hand=[], deck=[])
    state = GameState(players=[active, opponent], active_player_index=0)

    # WHEN play_card is called with card_index=5 (out of range)
    # THEN RuleViolationError("invalid card index") is raised
    with pytest.raises(RuleViolationError, match="invalid card index"):
        play_card(state, card_index=5)

    # AND the input state is unchanged
    assert state.players[0].hand == [2, 3]
    assert state.players[0].mana == 10
    assert state.players[1].hp == 30
